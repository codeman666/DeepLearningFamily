...
def parse_model(d, ch, verbose=True):
    ...
    """
    # 用于安全地把字符串转成 Python 对象。
    import ast
    # Args
    legacy = True  # backward compatibility for v3/v5/v8/v9 models 兼容旧版本模型结构。
    max_channels = float("inf") #限制每层最大输出通道。,inf 表示无限大。
    nc, act, scales = (d.get(x) for x in ("nc", "activation", "scales")) #调取模型参数。nc类别数，act激活函数，scales缩放比例。
    depth, width, kpt_shape = (d.get(x, 1.0) for x in ("depth_multiple", "width_multiple", "kpt_shape")) #调取模型参数默认为1。depth_multiple深度倍数，width_multiple宽度倍数，kpt_shape关键点形状。
    
    # 如果没有获取模型参数，则使用默认值
    if scales:
        scale = d.get("scale")
        if not scale:
            scale = tuple(scales.keys())[0] #如果没有获取到scale，则获取所有的key，然后进行排序，取第一个值
            LOGGER.warning(f"no model scale passed. Assuming scale='{scale}'.") # 在训练的时候打印出警告信息
        depth, width, max_channels = scales[scale] #获取指定 scale 的配置 解包赋值给三个变量，先找到模型对应的参数

    if act: 
        Conv.default_act = eval(act)  # 等价于直接写 nn.SiLU，返回 SiLU 类 redefine default activation, i.e. Conv.default_act = torch.nn.SiLU()
        if verbose: # 检查是否开启了详细输出模式
            LOGGER.info(f"{colorstr('activation:')} {act}")  # print
    # 打印模型结构
    if verbose:
        LOGGER.info(f"\n{'':>3}{'from':>20}{'n':>3}{'params':>10}  {'module':<45}{'arguments':<30}")
    
    ch = [ch] # 将通道数转换为列表（可以增加多个通道数）
    layers, save, c2 = [], [], ch[-1]  # layers, savelist, ch[-1]获取ch列表的最后一个元素
    base_modules = frozenset( # 定义一个 frozenset 对象，用于存储需要处理的模块
        {
            Classify,
            Conv,
            ConvTranspose,
    ...
    repeat_modules = frozenset(  # modules with 'repeat' arguments 不可变集合类型
        {
            BottleneckCSP,
            C1,
            C2,
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args   enumerate(...)的用法返回 (index, item) 的元组
        m = (
            # 三元函数
            getattr(torch.nn, m[3:]) #getattr() 从一个对象里“动态地”获取某个属性或者方法。
            if "nn." in m
            else getattr(__import__("torchvision").ops, m[16:]) #__import__("torchvision")动态导入等同于import torchvision
            if "torchvision.ops." in m
            else globals()[m] # 从当前 Python 文件的全局变量里找同名类。
        )  # get module
        
        for j, a in enumerate(args): #j 迭代参数列表 a 迭代参数
            if isinstance(a, str):
                with contextlib.suppress(ValueError): # 忽略ValueError
                    args[j] = locals()[a] if a in locals() else ast.literal_eval(a) #locals() 获取当前作用域的变量
