...
import val as validate  # for end-of-epoch mAP  每个训练周期（epoch）结束后，验证模型性能（mAP等）
from models.experimental import attempt_load # 自动加载模型文件（比如 yolov5s.pt）
from models.yolo import Model
from utils.autoanchor import check_anchors
from utils.autobatch import check_train_batch_size # 根据你的显卡内存自动调整 batch size，避免爆显存。
from utils.callbacks import Callbacks # 自定义 hook 函数插入训练流程
...
from utils.loss import ComputeLoss   # 定义 YOLOv5 的损失函数
from utils.metrics import fitness    # 用于进化算法里的综合评估函数
from utils.plots import plot_evolve  # 绘制超参数进化图（yolo 支持自动搜索最优超参数）
from utils.torch_utils import (
    EarlyStopping, # 当模型在验证集上的性能（比如 mAP）连续很多个 epoch 没有提升时，提前终止训练。
    ModelEMA,      # 对模型权重做滑动平均，可以让预测更加稳定、泛化更好
    de_parallel,
    select_device,
    smart_DDP,     # 根据当前设备状态，自动把模型转换成分布式训练结构，用于多GPU DDP（Distributed Data Parallel）。
    smart_optimizer,
    smart_resume, # 恢复训练时自动读取 .pt 权重文件和优化器状态，让训练从上次中断的地方继续
    torch_distributed_zero_first, # 在多个gpu训练的情况下，只允许第一个进程写入日志文件
)
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze = (
        Path(opt.save_dir),   # 存储模型输出的目录路径
        opt.epochs,           # 训练的总轮次
        opt.batch_size,       # 每个训练批次的样本数量
        opt.weights,          # 模型权重路径（预训练模型的路径）
        opt.single_cls,       # 是否只使用一个类进行训练
        opt.evolve,           # 是否开启进化算法
        opt.data,             # 数据集配置文件路径（如 coco128.yaml）
        opt.cfg,              # YOLO模型配置文件路径（如 yolov5s.yaml）
        opt.resume,           # 是否从上次训练中断的地方恢复训练
        opt.noval,            # 是否跳过验证阶段
        opt.nosave,           # 是否保存模型
        opt.workers,          # 数据加载的工作线程数
        opt.freeze,           # 是否冻结部分层（通常是冻结特征提取部分）
    )
    callbacks.run("on_pretrain_routine_start") # 自定义 hook 函数插入训练流程
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            pbar = tqdm(pbar, total=nb, bar_format=TQDM_BAR_FORMAT, dynamic_ncols=True)  # progress bar
        optimizer.zero_grad()
